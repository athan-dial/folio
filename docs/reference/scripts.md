...
```bash
python scripts/<script_name>.py <workspace_path>
```

```bash
bash scripts/compile_package.sh <workspace_path>
```

All workspace paths below are relative to your chosen workspace root unless stated otherwise. Run scripts from the repo root.
...
Runs from the repo root with workspace path. Expects `final/paper.tex` and `final/refs.bib`.
